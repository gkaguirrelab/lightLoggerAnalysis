...
% Collect minispect linearity recordings across NDFs and light levels.
%
% Syntax:
%   [success, ms_linearity_calibration_metadata] = collect_ms_linearity_data(device_num, ms_linearity_calibration_metadata, bluetooth_central, bluetooth_client, label, dropbox_savedir, local_savedir, cooldown_callback)
%
% Description:
%   This function coordinates the minispect linearity calibration by
%   iterating over NDF levels, randomized CombiLED intensity scalars, and
%   repeated measurements. For each requested condition it initializes the
%   CombiLED for the active NDF, applies a scaled background spectrum,
%   triggers a light-logger calibration recording over Bluetooth, waits
%   until the device returns to its idle state, and marks the condition as
%   completed in the metadata struct. The metadata is mutated in place so
%   interrupted runs can later be resumed without repeating completed
%   recordings.
%
% Inputs:
...
%                           for the minispect linearity stage.
...
% Examples:
%{
    [success, ms_meta] = collect_ms_linearity_data(device_num, ms_meta, ...
        bluetooth_central, bluetooth_client, "", cloud_output_dir, ...
        local_output_dir, @wait_for_combiLED_cooldown_between_NDFs);
%}
    ...
    ms_linearity_calibration_metadata.last_error_message = "";
    ...
    for nn = 1:numel(NDFs); 
            ...
            for setting_order_idx = 1:numel(settings_scalars_shuffled)
                ...
                try
                    ...
                    while(true)
                        ...
                        % as well 
                        if(state_name == "error") 
                            ms_linearity_calibration_metadata.last_error_message = describe_lightlogger_error(lightlogger_state);
                            success = 0 ; 
                            return ; 
                        end 
                    ...
                    report_bluetooth_failure("MS linearity", NDF, mm, ME);
                    ms_linearity_calibration_metadata.last_error_message = ...
                        sprintf("MS linearity bluetooth failure at NDF %.3f measurement %d.\n%s", ...
                                NDF, mm, getReport(ME, "extended", "hyperlinks", "off"));
                    success = 0;
                    return;
                end
        ...
        if(should_cooldown_before_next_NDF(ms_linearity_calibration_metadata.completed_measurements, nn) && ~isempty(cooldown_callback))
...
%   caller uses this to decide whether it is worth invoking the CombiLED
%   cooldown routine before proceeding.
% Inputs:
%   completed_measurements   - Logical completion tensor over NDF, setting,
%                              and repeat dimensions.
...
% Examples:
%{
    % See collect_ms_linearity_data.m for usage context.
%}
...
%
% Description:
%   This helper centralizes direct `py.getattr` access so the surrounding
%   calibration code can call Python-side functionality without repeating
%   the attribute lookup boilerplate.
% Inputs:
...
%   supplied arguments, keeping the MATLAB-side Bluetooth code concise and
%   consistent.
% Inputs:
%   module                   - Python module or object to inspect.
...
%   number of times before rethrowing the final exception so the caller can
%   distinguish between transient transport issues and persistent failures.
% Inputs:
...
%
% Syntax:
%   report_bluetooth_failure(measurement_name, NDF, measurement_num, ME)
%
% Description:
%   This helper formats the active measurement context together with the
%   full MATLAB exception report so failures can be diagnosed from console
%   logs after a calibration run aborts.
% Inputs:
%   measurement_name         - Label for the active calibration stage.
%   NDF                      - NDF value active when the failure occurred.
%   measurement_num          - Repeat index active when the failure
%                              occurred.
...
%}
    ...
    fprintf(2, "%s bluetooth failure at NDF %.3f measurement %d.\n", measurement_name, NDF, measurement_num);
...
%
% Description:
%   When the peripheral reports that it entered an error state, this
%   helper extracts high-level state fields, nested write-error metadata,
%   and a raw struct dump into one newline-delimited diagnostic string that
%   can be saved inside the calibration metadata.
% Inputs:
...
%   nested error payloads using ordinary MATLAB struct logic.
% Inputs:
...
%   `prefix.field = value` style string. It is used when serializing nested
%   Python error payloads into a compact human-readable report.
% Inputs:
