   ...
   fit_slopes_per_NDF = zeros(numel(NDFs), numel(frequencies));
    ...
    for ff = 1:numel(frequencies)
        ...
        for nn = 1:numel(NDFs)
            ...
            softenedColor = 1 - ((1 - colorList(nn,:)) * 0.4);
            plot(contrast_levels, mean_amplitudes_by_contrast, '-o', ...
                'Color', softenedColor, ...
                'MarkerFaceColor', softenedColor, ...
                'MarkerEdgeColor', softenedColor, ...
                'LineWidth', 1.5, ...
                'DisplayName', sprintf('NDF%.0f', NDF));

            fitCoeffs = polyfit(contrast_levels, mean_amplitudes_by_contrast, 1);
            fit_slopes_per_NDF(nn, ff) = fitCoeffs(1);
        end
