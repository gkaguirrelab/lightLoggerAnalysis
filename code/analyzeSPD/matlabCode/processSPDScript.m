...
activityLabels = {'walkIndoorFoveate','walkIndoorNoFoveate'};

for aa = 2:length(activityLabels)
    for ss = 1:length(subjectIDs)

        switch activityLabels{aa}
            case 'walkIndoorFoveate'
                thisFileName = ['FLIC_' subjectIDs{ss} '_walkIndoor_virtuallyFoveatedVideo.hdf5'];
                frameDropVector = [];
            case 'walkIndoorNoFoveate'
                thisFileName = ['FLIC_' subjectIDs{ss} '_walkIndoor_projectionNoFoveation.hdf5'];
                frameDropVector = activityData.walkIndoorFoveate.frameDropVector{ss};
        end

        thisVideo = fullfile(dropBoxDir,'FLIC_analysis','lightLogger',thisFileName);

        [activityData.(activityLabels{aa}).exponentMaps(:,:,ss), ...
            activityData.(activityLabels{aa}).interceptMaps(:,:,ss), ...
            activityData.(activityLabels{aa}).spdsByRegion(:,:,:,ss), frq, ...
            activityData.(activityLabels{aa}).medianImage(:,:,ss), ...
            activityData.(activityLabels{aa}).frameDropVector{ss}] = ...
            mapSPDs(thisVideo,'frameDropVector',frameDropVector);
    end

    avgExponentMap = squeeze(mean(activityData.(activityLabels{aa}).exponentMaps(:,:,1:4),3,'omitmissing'));
    avgVarianceMap = squeeze(mean(activityData.(activityLabels{aa}).interceptMaps(:,:,1:4),3,'omitmissing'));
    avgSpdByRegion = squeeze(mean(activityData.(activityLabels{aa}).spdsByRegion(:,:,:,1:4),4,'omitmissing'));
