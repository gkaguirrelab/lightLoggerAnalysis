...
% Get frequency bins
[~, frq] = calcTemporalSPD(v, fps, 'lineResolution', false, 'postreceptoralChannel', channel);
frq_bins = numel(frq);
...
for row = 1:step:(nRows - window(1) + 1)
    for col = 1:step:(nCols - window(2) + 1)
        ...
        try
            [spd, frq] = calcTemporalSPD(v, fps, 'lineResolution', false, 'postreceptoralChannel', channel, 'regionMatrix', regionMatrix);
        catch ME
