...
% Load in the world util library, which has some useful constants 
...
Pi_util = import_pyfile(getpref('lightLoggerAnalysis', 'world_util_path')); 
R_pixel_coords = Pi_util.WORLD_R_PIXELS + 1; % + 1 here since Python indices are 0 indexed and MATLAB is 1 
B_pixel_coords = Pi_util.WORLD_G_PIXELS + 1; % they are in the shape [ (row, col)  ]

switch pixelClass
    case 'R' % You might have to ensure pixel class is upper case if you're going to do this (not entirely sure with MATLAB)
             % Underlyingly, 'r' and 'R' are different ascii int values, so this switch may not work with 'r'
        for rr = 1:nRows; 
            for cc = 1:nCols;
