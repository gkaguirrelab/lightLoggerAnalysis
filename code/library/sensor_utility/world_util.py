...
@njit(inline="always")
def _clamp_index(index: int, upper_bound: int) -> int:
    """Clamp a pixel index to ``[0, upper_bound - 1]``."""
    if(index < 0):
        return 0
...
@njit(inline="always")
def _average_two_inf_aware(first_value: float,
                           second_value: float,
                           ) -> float:
    """Average two samples, propagating Inf instead of diluting it."""
    if(np.isinf(first_value) or np.isinf(second_value)):
        return np.inf
    return (first_value + second_value) / 2.0


@njit(inline="always")
def _average_four_inf_aware(first_value: float,
                            second_value: float,
                            third_value: float,
                            fourth_value: float,
                            ) -> float:
    """Average four samples, propagating Inf instead of diluting it."""
    if(
        np.isinf(first_value)
        or np.isinf(second_value)
        or np.isinf(third_value)
        or np.isinf(fourth_value)
    ):
        return np.inf
    return (first_value + second_value + third_value + fourth_value) / 4.0


@njit(inline="always")
def _local_three_by_three_contains_inf(raw_frame_buffer: np.ndarray,
                                       frame_index: int,
                                       center_row: int,
                                       center_col: int,
                                       up_row: int,
    ...
    """Return whether the standard local 3x3 Bayer neighborhood has Inf."""
    return (
        np.isinf(raw_frame_buffer[frame_index, center_row, center_col])
        or np.isinf(raw_frame_buffer[frame_index, up_row, center_col])
        or np.isinf(raw_frame_buffer[frame_index, down_row, center_col])
        or np.isinf(raw_frame_buffer[frame_index, center_row, left_col])
        or np.isinf(raw_frame_buffer[frame_index, center_row, right_col])
        or np.isinf(raw_frame_buffer[frame_index, up_row, left_col])
        or np.isinf(raw_frame_buffer[frame_index, up_row, right_col])
        or np.isinf(raw_frame_buffer[frame_index, down_row, left_col])
        or np.isinf(raw_frame_buffer[frame_index, down_row, right_col])
    )


@njit(parallel=True)
def _debayer_inf_aware_compiled(raw_frame_buffer: np.ndarray,
                                debayered_frame_buffer: np.ndarray,
                                ) -> None:
    """Inf-aware clone of OpenCV's bilinear ``COLOR_BayerRG2RGB`` path.

    The finite-value interpolation follows the Bayer layout used elsewhere
    in this module: blue at even/even sites, red at odd/odd sites, and green
    at mixed-parity sites. Border pixels are mapped to the adjacent interior
    pixel, matching the border-copy behavior used by OpenCV's bilinear Bayer
    conversion. Any Inf in the standard local 3x3 Bayer neighborhood makes
    the full output RGB pixel Inf, including at green sites where OpenCV's
    finite bilinear interpolation otherwise uses a smaller cross-shaped
    support.
    """
    n_frames: int = raw_frame_buffer.shape[0]
    rows: int = raw_frame_buffer.shape[1]
    cols: int = raw_frame_buffer.shape[2]

    for frame_row_index in prange(n_frames * rows):
        frame_index: int = frame_row_index // rows
        output_row: int = frame_row_index % rows

        for output_col in range(cols):
            source_row: int = output_row
            source_col: int = output_col

            if(rows > 1):
                if(source_row == 0):
                    source_row = 1
                elif(source_row == rows - 1):
                    source_row = rows - 2

            if(cols > 1):
                if(source_col == 0):
                    source_col = 1
                elif(source_col == cols - 1):
                    source_col = cols - 2

            up_row: int = _clamp_index(source_row - 1, rows)
            down_row: int = _clamp_index(source_row + 1, rows)
            left_col: int = _clamp_index(source_col - 1, cols)
            right_col: int = _clamp_index(source_col + 1, cols)

            center_value: float = raw_frame_buffer[frame_index, source_row, source_col]
            if(_local_three_by_three_contains_inf(
                raw_frame_buffer,
                frame_index,
                source_row,
                source_col,
                ...
                debayered_frame_buffer[frame_index, output_row, output_col, 0] = np.inf
                debayered_frame_buffer[frame_index, output_row, output_col, 1] = np.inf
                debayered_frame_buffer[frame_index, output_row, output_col, 2] = np.inf
                continue

            row_even: bool = (source_row % 2 == 0)
            col_even: bool = (source_col % 2 == 0)

            if(row_even and col_even):
                # Blue Bayer site.
                debayered_frame_buffer[frame_index, output_row, output_col, 0] = _average_four_inf_aware(
                    raw_frame_buffer[frame_index, up_row, left_col],
                    raw_frame_buffer[frame_index, up_row, right_col],
                    raw_frame_buffer[frame_index, down_row, left_col],
                    raw_frame_buffer[frame_index, down_row, right_col],
                )
                debayered_frame_buffer[frame_index, output_row, output_col, 1] = _average_four_inf_aware(
                    raw_frame_buffer[frame_index, up_row, source_col],
                    raw_frame_buffer[frame_index, down_row, source_col],
                    raw_frame_buffer[frame_index, source_row, left_col],
                    raw_frame_buffer[frame_index, source_row, right_col],
                )
                debayered_frame_buffer[frame_index, output_row, output_col, 2] = center_value

            elif((not row_even) and (not col_even)):
                # Red Bayer site.
                debayered_frame_buffer[frame_index, output_row, output_col, 0] = center_value
                debayered_frame_buffer[frame_index, output_row, output_col, 1] = _average_four_inf_aware(
                    raw_frame_buffer[frame_index, up_row, source_col],
                    raw_frame_buffer[frame_index, down_row, source_col],
                    raw_frame_buffer[frame_index, source_row, left_col],
                ...
                debayered_frame_buffer[frame_index, output_row, output_col, 2] = _average_four_inf_aware(
                    raw_frame_buffer[frame_index, up_row, left_col],
                    raw_frame_buffer[frame_index, up_row, right_col],
                    raw_frame_buffer[frame_index, down_row, left_col],
                    raw_frame_buffer[frame_index, down_row, right_col],
                ...
                # Green Bayer site on a blue row.
                debayered_frame_buffer[frame_index, output_row, output_col, 0] = _average_two_inf_aware(
                    raw_frame_buffer[frame_index, up_row, source_col],
                    raw_frame_buffer[frame_index, down_row, source_col],
                )
                debayered_frame_buffer[frame_index, output_row, output_col, 1] = center_value
                debayered_frame_buffer[frame_index, output_row, output_col, 2] = _average_two_inf_aware(
                    raw_frame_buffer[frame_index, source_row, left_col],
                    raw_frame_buffer[frame_index, source_row, right_col],
                )

            else:
                # Green Bayer site on a red row.
                debayered_frame_buffer[frame_index, output_row, output_col, 0] = _average_two_inf_aware(
                    raw_frame_buffer[frame_index, source_row, left_col],
                    raw_frame_buffer[frame_index, source_row, right_col],
                )
...
def debayer_inf_aware(image_or_video: np.ndarray,
                      visualize_results: bool=False
                      ) -> np.ndarray | tuple[np.ndarray, object]:
    """Debayer raw Bayer data while preserving Inf saturation markers.

    This is a floating-point, Inf-aware implementation of the bilinear
    interpolation used by :func:`debayer`. It is intended for diagnostic
    processing stages where saturated Bayer samples have already been marked
    as ``np.inf`` and should remain saturated after interpolation. The finite
    output follows OpenCV's bilinear formulas, but Inf propagation is
    deliberately conservative: any Inf in the standard local 3x3 Bayer
    neighborhood marks the entire output RGB pixel as Inf.

    Args:
        image_or_video: A single raw Bayer frame with shape ``(rows, cols)``
            or raw Bayer frame buffer with shape ``(frames, rows, cols)``.
        visualize_results: When ``True``, display a before/after figure and
            ...
            a single ``(rows, cols)`` frame.

    Returns:
        A ``float64`` RGB image or frame buffer. Any output pixel with an Inf
        in its local 3x3 Bayer neighborhood is set to ``np.inf`` in all three
        RGB channels.
    """
    ...
    if(visualize_results is True):
        assert image_or_video.ndim == 2, (
            f"Inf-aware debayer visualization only supports a single frame "
    ...
    if(image_or_video.ndim == 2):
        raw_frame_buffer = image_or_video[np.newaxis, ...].astype(np.float64, copy=False)
    elif(image_or_video.ndim == 3):
        raw_frame_buffer = image_or_video.astype(np.float64, copy=False)
    else:
        raise Exception(f"Unsupported N Dimensions: {image_or_video.ndim}. N dimensions must be 2 or 3")

    assert raw_frame_buffer.shape[1] >= 2 and raw_frame_buffer.shape[2] >= 2, (
        "Inf-aware debayering requires at least two rows and two columns. "
        f"Got shape {image_or_video.shape}."
    )

    debayered_frame_buffer: np.ndarray = np.empty(
        tuple(list(raw_frame_buffer.shape) + [3]),
        dtype=np.float64,
    )
    _debayer_inf_aware_compiled(raw_frame_buffer, debayered_frame_buffer)

    debayered = debayered_frame_buffer[0] if image_or_video.ndim == 2 else debayered_frame_buffer

    if(visualize_results is True):
        fig, axes = plt.subplots(1, 2, figsize=(12, 4))
        fig.suptitle("Inf-Aware Debayering (Before / After)", fontweight='bold', fontsize=18)

        axes[0].imshow(unmodified_image_or_video, cmap="gray")
        axes[0].set_title("Before")
        axes[0].axis("off")

        finite_after = debayered[np.isfinite(debayered)]
        after_display_max = np.max(finite_after) if finite_after.size > 0 else 1
        axes[1].imshow(np.clip(np.nan_to_num(debayered, posinf=after_display_max) / after_display_max, 0, 1))
...
@njit(inline="always")
def _resolve_channel_from_accumulators(valid_total: float,
                                       valid_count: int,
                                       dropped_floor: bool,
                                       ) -> float:
    """Resolve one debayered channel from its patch accumulators.

    Given the running sum/count of the valid (non-sentinel) same-colored
    contributors found in a pixel's local patch, plus whether any floor marker
    had to be dropped, return the channel value used by
    :func:`_debayer_custom_compiled`.

    Args:
        valid_total: Sum of the contributors that were neither ``np.inf``
            (ceiling) nor ``0.0`` (floor).
        valid_count: Number of those valid contributors.
        dropped_floor: Whether at least one contributor of this color was a
            floor (``0.0``) marker.

    Returns:
        The mean of the valid contributors when any exist. Otherwise a fallback
        sentinel: ``0.0`` if a floor marker was among the (all-invalid)
        contributors -- floor takes precedence, matching
        :func:`apply_floor_ceiling` -- else ``np.inf``.
    """
    # If at least one same-colored patch member carried real signal, the
    # channel is simply their mean.
    if(valid_count > 0):
        return valid_total / valid_count

    # No valid contributor of this color survived, so the channel is uninformed
    # and falls back to a sentinel. Because every color is present in a full 3x3
    # patch, reaching here means every same-colored member was a marker: emit
    # floor when any floor was seen (floor precedence), otherwise ceiling.
    if(dropped_floor):
        return 0.0
    return np.inf
...
@njit(parallel=True)
def _debayer_custom_compiled(raw_frame_buffer: np.ndarray,
                             debayered_frame_buffer: np.ndarray,
                             ) -> None:
    """Averaging de-Bayer using one uniform 3x3 patch for every pixel.

    Every output pixel is treated identically: gather the full local 3x3 patch
    around the (border-adjusted) source pixel and, for each RGB channel, average
    all patch members whose Bayer color matches that channel. There is no
    special cross-shaped support for green sites and no direct copy of the
    center sample -- the *same* patch and the *same* averaging rule are used for
    red, green, and blue, so the only thing that varies between site types is how
    many members of each color the fixed Bayer geometry happens to place in the
    patch.

    Saturation is handled per color exactly as before: ceiling (``np.inf``) and
    floor (``0.0``) members are excluded from the mean, and a channel only
    becomes a sentinel when none of its same-colored patch members are valid
    (see :func:`_resolve_channel_from_accumulators`).

    Note that under this uniform rule a green site's own green channel is the
    mean of the five greens in its patch (its center plus four diagonal greens)
    rather than its measured center value, which is the intended consequence of
    not treating green differently. Red and blue sites are unchanged from a
    center-copy scheme because a 3x3 patch around a red/blue center contains
    exactly one pixel of the center color.

    Args:
        raw_frame_buffer: Float raw Bayer buffer of shape
            ``(frames, rows, cols)``. Read-only; ``np.inf`` marks ceiling
            saturation and ``0.0`` marks floor/dark samples.
        debayered_frame_buffer: Pre-allocated float output buffer of shape
            ``(frames, rows, cols, 3)`` written in place with RGB values.
    """
    n_frames: int = raw_frame_buffer.shape[0]
    rows: int = raw_frame_buffer.shape[1]
    cols: int = raw_frame_buffer.shape[2]

    # Flatten the (frame, row) iteration space so Numba can parallelize across
    # every scanline of every frame at once.
    for frame_row_index in prange(n_frames * rows):
        frame_index: int = frame_row_index // rows
        output_row: int = frame_row_index % rows

        for output_col in range(cols):
            # Map border output pixels onto the nearest interior pixel so the
            # patch parity matches OpenCV's border-copy behavior (identical to
            # the inf-aware kernel).
            source_row: int = output_row
            source_col: int = output_col
            ...
            # Per-color accumulators over the full 3x3 patch: running sum and
            # count of valid (non-sentinel) members, plus a flag recording
            # whether any floor (0.0) member of that color was dropped.
            red_total: float = 0.0
            red_count: int = 0
            red_dropped_floor: bool = False

            green_total: float = 0.0
            green_count: int = 0
            green_dropped_floor: bool = False

            blue_total: float = 0.0
            blue_count: int = 0
            blue_dropped_floor: bool = False

            # Walk the same 3x3 patch for every pixel. Clamp so the patch stays
            # in bounds on tiny frames; clamping only ever re-counts an existing
            # in-patch pixel, never invents a new color.
            for row_offset in range(-1, 2):
                neighbor_row: int = _clamp_index(source_row + row_offset, rows)
                neighbor_row_even: bool = (neighbor_row % 2 == 0)

                for col_offset in range(-1, 2):
                    neighbor_col: int = _clamp_index(source_col + col_offset, cols)
                    neighbor_col_even: bool = (neighbor_col % 2 == 0)

                    value: float = raw_frame_buffer[frame_index, neighbor_row, neighbor_col]

                    # Route this patch member into the accumulator for its Bayer
                    # color (blue at even/even, red at odd/odd, green on mixed
                    # parity), excluding ceiling/floor markers from the mean.
                    if(neighbor_row_even and neighbor_col_even):
                        # Blue member.
                        if(np.isinf(value)):
                            pass
                        elif(value == 0.0):
                            blue_dropped_floor = True
                        else:
                            blue_total += value
                            blue_count += 1

                    elif((not neighbor_row_even) and (not neighbor_col_even)):
                        # Red member.
                        if(np.isinf(value)):
                            pass
                        elif(value == 0.0):
                            red_dropped_floor = True
                        else:
                            red_total += value
                            red_count += 1
                        ...
                        if(np.isinf(value)):
                            ...
                            green_dropped_floor = True
                        else:
                            green_total += value
                            green_count += 1

            # Resolve each channel from its accumulated same-colored members.
            debayered_frame_buffer[frame_index, output_row, output_col, 0] = _resolve_channel_from_accumulators(
                red_total, red_count, red_dropped_floor,
            )
            debayered_frame_buffer[frame_index, output_row, output_col, 1] = _resolve_channel_from_accumulators(
                green_total, green_count, green_dropped_floor,
            )
            debayered_frame_buffer[frame_index, output_row, output_col, 2] = _resolve_channel_from_accumulators(
                blue_total, blue_count, blue_dropped_floor,
            )

    return


def debayer_custom(image_or_video: np.ndarray,
                   visualize_results: bool=False
                   ) -> np.ndarray | tuple[np.ndarray, object]:
    """Debayer float Bayer data by averaging only unsaturated neighbors.

    This is a parallel implementation to :func:`debayer` that mirrors its public
    structure (single-frame vs. frame-buffer handling and the optional
    before/after visualization) but replaces the OpenCV call with a manual,
    fully floating-point demosaic. Unlike :func:`debayer` and
    :func:`debayer_inf_aware`, every pixel is demosaiced with one *uniform* rule:
    take the full local 3x3 patch and set each RGB channel to the average of the
    patch members whose Bayer color matches that channel. Green sites are not
    treated differently -- there is no cross-shaped support and no direct copy of
    the measured center -- so the only thing that varies between site types is how
    many members of each color the fixed Bayer geometry places in the patch.

    Saturation is handled per channel, the opposite of
    :func:`debayer_inf_aware`'s conservatism:

        - :func:`debayer_inf_aware` -- a single ``np.inf`` in the local Bayer
          neighborhood forces the entire output RGB pixel to Inf.
        - :func:`debayer_custom` -- each channel averages only its *valid*
          same-colored patch members, where a member is invalid if it is a
          ceiling marker (``np.inf``) or a floor marker (``0.0``). A channel is
          only marked as a sentinel when it has no valid member at all.

    Concretely, if a channel draws on four same-colored patch members and one of
    them is saturated, the output is the average of the remaining three. If none
    of a channel's members are valid, the channel falls back to a sentinel:
    ``0.0`` when a floor marker was among the dropped members (floor takes
    precedence, matching :func:`apply_floor_ceiling`), otherwise ``np.inf``.
    Because green appears at five sites within a green-centered patch, a green
    site's green channel is the mean of those five greens rather than its
    measured center value -- the intended consequence of the uniform rule.

    Because saturation is encoded as ``np.inf``, the input must be
    floating-point -- integer arrays cannot represent the ceiling marker, so
    accepting only float data is what makes the sentinels detectable in the
    first place.

    Args:
        image_or_video: A single raw Bayer frame with shape ``(rows, cols)`` or
            a raw Bayer frame buffer with shape ``(frames, rows, cols)``. Must be
            a floating-point dtype. ``np.inf`` marks ceiling-saturated samples
            and ``0.0`` marks floor/dark samples.
        visualize_results: When ``True``, display a before/after figure and
            return it with the debayered result. Visualization supports only a
            single ``(rows, cols)`` frame and asserts otherwise.

    Returns:
        A ``float64`` RGB image or frame buffer. When ``visualize_results`` is
        ``True``, returns ``(debayered, figure)``.

    Raises:
        AssertionError: If the input is not floating-point, or if a frame has
            fewer than two rows or columns.
        Exception: If the input does not have 2 or 3 dimensions.
    """
    image_or_video = np.asarray(image_or_video)

    # This variant only makes sense on floating-point data: the ceiling marker
    # is np.inf, which integer arrays cannot represent. Requiring a float dtype
    # is what lets us detect saturated samples at all.
    assert np.issubdtype(image_or_video.dtype, np.floating), (
        "debayer_custom requires a floating-point image or video so that np.inf "
        f"ceiling markers can be detected. Got dtype {image_or_video.dtype}."
    )

    unmodified_image_or_video: np.ndarray | None = None
    if(visualize_results is True):
        assert image_or_video.ndim == 2, (
            f"Custom debayer visualization only supports a single frame with "
            f"shape (rows, cols). Got shape {image_or_video.shape}."
        )
        unmodified_image_or_video = image_or_video.copy()

    # Normalize single-image and video inputs to a (frames, rows, cols) buffer
    # for the compiled kernel. astype(copy=False) avoids a copy when the input
    # is already float64; the kernel only reads this buffer, so the caller's
    ...
    assert raw_frame_buffer.shape[1] >= 2 and raw_frame_buffer.shape[2] >= 2, (
        "Custom debayering requires at least two rows and two columns. "
        f"Got shape {image_or_video.shape}."
    )

    # Allocate the RGB output buffer (input shape + a trailing channel axis).
    debayered_frame_buffer: np.ndarray = np.empty(
        tuple(list(raw_frame_buffer.shape) + [3]),
        dtype=np.float64,
    )
    _debayer_custom_compiled(raw_frame_buffer, debayered_frame_buffer)

    # Collapse the leading frame axis back out for single-image inputs.
    debayered = debayered_frame_buffer[0] if image_or_video.ndim == 2 else debayered_frame_buffer

    # Visualize the results if desired. Inf pixels are mapped to the finite max
    # so the "after" panel remains displayable.
    if(visualize_results is True):
        fig, axes = plt.subplots(1, 2, figsize=(12, 4))
        fig.suptitle("Custom Debayering (Before / After)", fontweight='bold', fontsize=18)
